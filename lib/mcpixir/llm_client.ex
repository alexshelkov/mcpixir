defmodule Mcpixir.LLMClient do
  ...
  def run(client, messages) do
    ...
    if provider_module do
      ...
      {:ok,
       %{
         "role" => "assistant",
         "content" => "LLM integration is not available. Using placeholder response."
       }}
    end
  end
  ...
  defp get_provider_module(provider) do
    alias Mcpixir.LLMClient.{Anthropic, LangChain, OpenAI}

    case provider do
      :openai ->
        if Code.ensure_loaded?(OpenAI), do: OpenAI, else: nil

      :anthropic ->
        if Code.ensure_loaded?(Anthropic), do: Anthropic, else: nil

      :langchain ->
        if Code.ensure_loaded?(LangChain), do: LangChain, else: nil
