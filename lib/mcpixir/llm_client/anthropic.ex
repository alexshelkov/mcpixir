defmodule Mcpixir.LLMClient.Anthropic do
  @moduledoc """
  Anthropic-specific implementation of the LLM client.

  This module handles communication with Anthropic's API (Claude models),
  using the LangChain library with support for tool calling.
  """
  ...
  def run(client, messages) do
    if langchain_available?() do
      ...
      {:ok,
       %{"role" => "assistant", "content" => "Anthropic integration requires LangChain library."}}
    end
  end

  defp run_with_langchain(client, messages) do
    model = Map.get(client.config, :model, "claude-3-sonnet-20240229")
